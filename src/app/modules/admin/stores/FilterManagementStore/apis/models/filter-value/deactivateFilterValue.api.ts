...
export interface DeactivateFilterValueRequestPathParamsDTO {
  filterValueId: number;
}

export interface DeactivateFilterValueDTO {
  filterValueId: number;
  isActive: false;
}

export type DeactivateFilterValueSuccessResponseDTO = ApiSuccessResponseBody<DeactivateFilterValueDTO>;

export class FilterValueAlreadyInactiveException extends ApiException {
  constructor(code: string, message: string) {
    super(409, code, message);
    this.name = "FilterValueAlreadyInactiveException";
  }
}

export function mapDeactivateFilterValueError(err: ApiError): ApiException {
  if (err.kind === "network") {
  ...
  switch (err.code) {
    ...
    case "FILTER_VALUE.ALREADY_INACTIVE":
      return new FilterValueAlreadyInactiveException(err.code, err.message);

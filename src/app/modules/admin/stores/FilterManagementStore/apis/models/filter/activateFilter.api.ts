...
export interface ActivateFilterRequestPathParamsDTO {
  filterId: number;
}

export interface ActivateFilterDTO {
  filterId: number;
  isActive: true;
  note: string;
}

export type ActivateFilterSuccessResponseDTO = ApiSuccessResponseBody<ActivateFilterDTO>;

export class FilterAlreadyActiveException extends ApiException {
  constructor(code: string, message: string) {
    super(409, code, message);
    this.name = "FilterAlreadyActiveException";
  }
}

export function mapActivateFilterError(err: ApiError): ApiException {
  if (err.kind === "network") {
  ...
  switch (err.code) {
    ...
    case "FILTER.ALREADY_ACTIVE":
      return new FilterAlreadyActiveException(err.code, err.message);
